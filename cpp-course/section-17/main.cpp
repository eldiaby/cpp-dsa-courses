...
void fill(std::vector<std::shared_ptr<Test>> &vec, int num)
{
    vec.reserve(static_cast<size_t>(num));
    for (int i = 0; i < num; i++)
        vec.push_back(std::make_shared<Test>(get_input(i + 1)));
}

int get_input(const int index)
{
    std::cout << "Enter data point [" << index << "] : ";
    int user_input{};
    std::cin >> user_input;
    return user_input;
}
...
void display(const std::vector<std::shared_ptr<Test>> &vec)
{
    ...
    for (const auto &item : vec)
        std::cout << item->get_data() << std::endl;
